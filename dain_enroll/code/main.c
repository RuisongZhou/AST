...
#define GR_H 32
#define GR_W 32  //Ä¬ÈÏµÄÑµÁ·¼¯ºÍ²âÊÔ¼¯µÄ¿íºÍ¸ß

#define VECTOR_H 2
#define VECTOR_W 2  //Ò»¿éÈ¡ÌØÕ÷ÖµµÄÇøÓòµÄ¿íºÍ¸ß

typedef struct {
	int w, h;
	int a[GR_H][GR_W]; //Ô­Í¼·Ö¸î³Éw*hµÄ
} graph;
...
float cal_distance(graph *a, graph *b)   //¼ÆËãÅ·ÊÏ¾àÀë
{
	int sum = 0, r, c, t;
	for (r = 0; r < a->h; r++)
		for (c = 0; c < a->w; c++) {
...
int MakeAnswer(graph *g) //Ê¶±ð½á¹û
{
	int i, j;
	int SumCount[10];
	int t1 = 0, t2 = 0;
...
int read_file_list(char *path)  //´¦ÀíÑµÁ·Êý¾Ý
{
	long Handle;
	int i = 0;
	char Spath[100];
...
void work(char *path) //´¦Àí²âÊÔÊý¾Ý
{
	long Handle;
	//int i = 0;
	char Spath[100];
...
void read_a_single_file(char *f_path, graph *g)     //´¦Àí²¢±£´æÊý¾Ý
{
	FILE *f;
	int w, h, i, j, r, c, *p;
	f = fopen(f_path, "r");
	...
	h = 0;
	while (fscanf(f, "%s", tmp_init_s[h++]) == 1); //¶ÁÈëÃ¿ÐÐÊý¾Ý
	w = strlen(tmp_init_s[0]);  //Ã¿ÐÐ³¤¶È
	g->w = w / VECTOR_W;    //WÊÇ¿í¶È
	g->h = h / VECTOR_H;    //hÊÇ¸ß¶È
